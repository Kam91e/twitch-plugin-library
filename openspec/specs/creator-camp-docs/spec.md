...
Provide a consolidated, source-traced documentation surface at `docs/creator-camp/` (alias `knowledge/creator-camp/`) summarizing all text-based Creator Camp lessons (Level 1 + Level 2, excluding Live Learning video sessions) via Chrome-based extraction. Each path file mixes path synthesis (Option B), per-chapter TL;DR (Option A), and sortable tables (Option D), with every fact traced to its canonical `https://www.twitch.tv/creatorcamp/...` URL.
